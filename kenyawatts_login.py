...
def save_submission(record):
    ...
    If the county submits again, the new record REPLACES the old one
    for the same county. This means:
    - The validation queue shows the latest version only
    - The national aggregation uses the latest figures
    - The county's submission history shows the most recent plan
    - Previous EPRA approval status is reset to "submitted" for re-review

    This is the correct behaviour — a resubmission should trigger
    a fresh EPRA review with the updated data.
    """
    if USE_CLOUD_DB:
    ...
    with _sub_lock:
        ...
        # Remove ALL previous submissions from the same county
        # (not just same ref) so resubmissions fully replace old data
        county_name = record.get("county","")
        if county_name:
            subs = [s for s in subs if s.get("county","") != county_name]
        else:
...
def main():
    ...
    # ── Floating AI assistant widget (appears on every page) ──────────────────
    # This renders AFTER the page content so it sits on top of everything.
    # It uses Streamlit's chat_input which sticks to the bottom of the app.
    # The toggle button uses session state to show/hide the chat panel.
    #
    # HOW IT WORKS:
    # - A floating ⚡ button sits in the bottom-right corner (pure CSS)
    # - Clicking it toggles st.session_state["ai_widget_open"]
    # - When open, the chat history and input are shown in a panel
    # - The AI uses the same ask_ai() function as the full AI page
    _render_floating_ai(role, county_id)


def _render_floating_ai(role, county_id):
    """
    Render the floating AI chat widget that appears on every page.
    Uses a combination of CSS positioning and Streamlit expander to
    create a bottom-right chat panel. The toggle state is kept in
    session state so it persists while the user navigates between pages.
    """
    ...
    # Toggle button — fixed position bottom right via CSS
    # We use an HTML button that triggers a Streamlit button via JS workaround:
    # render a real st.button hidden behind an absolutely-positioned styled div
    toggle_label = "✕ Close AI" if st.session_state["ai_open"] else "⚡ AI"

    # Place toggle in a container that we CSS-position to bottom-right
    st.markdown("""
    <style>
    /* Hide the default Streamlit button styling for our AI toggle */
    div[data-testid="stButton"][data-key="ai_toggle_btn"] button {
        ...
        background: linear-gradient(135deg, #0e1e2e, #1a6fa3) !important;
        color: white !important;
        font-size: 20px !important;
        border: 2px solid rgba(62,207,170,0.6) !important;
        box-shadow: 0 4px 20px rgba(26,111,163,0.5) !important;
        padding: 0 !important;
    }
    div[data-testid="stButton"][data-key="ai_toggle_btn"] button:hover {
        transform: scale(1.1) !important;
        box-shadow: 0 6px 28px rgba(26,111,163,0.7) !important;
    }
    </style>
    """, unsafe_allow_html=True)

    if st.button("⚡", key="ai_toggle_btn", help="Open KenyaWatts AI assistant"):
        st.session_state["ai_open"] = not st.session_state["ai_open"]
        st.rerun()

    # Chat panel — shown when ai_open is True
    if st.session_state["ai_open"]:
        st.markdown("""
        <style>
        /* Position the chat panel above the toggle button */
        section[data-testid="stMain"] > div > div:last-child {
            position: fixed !important;
            ...
            border: 1px solid rgba(62,207,170,0.3) !important;
        ...
        # Panel header
        st.markdown("""
        <div style="background:#0e1e2e;border-radius:14px 14px 0 0;padding:12px 16px;
             border-bottom:0.5px solid rgba(255,255,255,0.1);
             font-size:13px;font-weight:600;color:#ffffff;">
          ⚡ KenyaWatts AI assistant
          <span style="font-size:11px;color:#5a8a9e;margin-left:8px;font-weight:400;">
            Powered by Claude
          </span>
        </div>
        """, unsafe_allow_html=True)
        ...
        if not st.session_state["ai_history"]:
            suggestions = [
                "Which counties need most urgent help?",
                "How is Kenya tracking to 2030 access target?",
                "What does Makueni CEP tell us about clean cooking?",
                "How does the national aggregation work?",
            ]
            st.markdown(
            ...
            for i, s in enumerate(suggestions):
                if st.button(s, key=f"ai_fab_sug_{i}",
                             use_container_width=True):
        ...
        # Chat history
        chat_html = "<div style='padding:8px 12px;max-height:300px;overflow-y:auto'>"
        for msg in st.session_state["ai_history"][-8:]:  # show last 8 messages
            if msg["role"] == "user":
                chat_html += (
                    f"<div style='background:#1a6fa3;color:#fff;border-radius:12px 12px 2px 12px;"
                    f"padding:8px 12px;margin-bottom:8px;font-size:12px;line-height:1.5;"
                    f"max-width:85%;margin-left:auto'>{msg['content']}</div>"
                )
            else:
                chat_html += (
                    f"<div style='background:rgba(255,255,255,0.08);color:#e8e6e0;"
                    f"border-radius:12px 12px 12px 2px;padding:8px 12px;margin-bottom:8px;"
                    f"font-size:12px;line-height:1.5;max-width:90%'>"
                    f"<span style='color:#3ecfaa;font-weight:600'>⚡ </span>"
                    f"{msg['content']}</div>"
                )
        chat_html += "</div>"
        ...
        if ci2.button("Send", key="ai_fab_send", type="primary"):
            if user_input.strip():
                st.session_state["ai_history"].append(
        ...
        # Clear button
        if st.session_state["ai_history"]:
            if st.button("Clear chat", key="ai_fab_clear",
                         use_container_width=True):
                st.session_state["ai_history"] = []
                st.rerun()
