...
<style>
  ...
  [data-testid="stMain"] > div {contain:layout style;overflow:hidden}
...
def main():
    ...
    # ── Track page changes and force ONE clean rerun ──────────────────────────
    # This clears all previous page widgets before rendering the new page.
    # Without this Streamlit stacks new content on top of old content.
    prev = st.session_state.get("_page_prev", None)
    if prev is not None and prev != selected:
        # Page changed — store new selection and rerun cleanly
        st.session_state["_page_prev"] = selected
        st.session_state["_page_changed"] = True
        st.rerun()
    elif st.session_state.get("_page_changed"):
        # This is the clean rerun after page change — clear the flag
        st.session_state["_page_changed"] = False
    else:
        # First load or same page — just store current
        st.session_state["_page_prev"] = selected
    ...
    def _matches(*terms):
        return any(t.lower() in key_lower or t.lower() in selected.lower() for t in terms)

    with st.container():
        if _matches("National overview") and not _matches(
            "Communications","Validation","Submit","Download",
            "AI","Account","Makueni","INEP report","County map",
            "47 counties","dashboard","County demand"
        ):
            page_national_overview(role)
        elif _matches("County map") and not _matches(
            "Communications","47 counties","demand data"
        ):
            page_map(role, county_id)
        elif _matches("All 47","County demand"):
            page_all_counties()
        elif _matches("dashboard"):
            page_county_dashboard(county_id, user_info["name"])
        elif _matches("Validation queue"):
            page_validation_queue()
        elif _matches("Submit"):
            page_submit(role, county_id, user_info["name"])
        elif _matches("Communications hub","Communications","County inbox"):
            page_inbox(role, county_id, user_info["name"])
        elif _matches("Makueni CEP","My county plan","Makueni reference","template reference"):
            page_makueni_contextual(role, county_id)
        elif _matches("National INEP report","INEP report"):
            page_national_report(user_info)
        elif _matches("Data download"):
            page_data_download(role, county_id)
        elif _matches("AI assistant","AI"):
            page_ai(role, county_id)
        elif _matches("Account settings"):
