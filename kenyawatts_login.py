...
def main():
    ...
    if role in ("epra", "developer"):
        ...
        base_pages = [
            ...
            # AI assistant intentionally excluded — it lives as a floating widget
            st.Page(_account,           title="Account settings",                 icon="⚙️"),
        ]
        # Developer tools tab only for developer role (not for EPRA)
        if role == "developer":
            pages = base_pages + [
                st.Page(_dev_tools, title="Developer tools", icon="🔧"),
            ]
        else:
            pages = base_pages
    elif role == "ministry":
...
def _render_floating_ai(role, county_id):
    """
    Render the AI assistant as a sidebar widget that always appears at the
    bottom of the sidebar, below the navigation links and sign out button.

    WHY SIDEBAR INSTEAD OF FLOATING BUTTON:
    Streamlit runs inside an iframe and its security model blocks JavaScript
    from toggling session state via DOM manipulation. A floating HTML button
    cannot reliably trigger a Streamlit rerun. The sidebar is Streamlit-native
    and always works — the AI chat opens/closes cleanly with no JS hacks.

    The AI panel appears as an expander pinned to the bottom of the sidebar.
    It persists across page navigation because it is in main(), not in any
    individual page function.
    """
    ...
    with st.sidebar:
        ...
        # The expander IS the toggle — clicking it opens/closes the chat
        with st.expander("Ask KenyaWatts AI", expanded=st.session_state.get("ai_open", False)):
            st.session_state["ai_open"] = True   # mark as opened when expander is used

            # Quick suggestion buttons (only when chat is empty)
            if not st.session_state["ai_history"]:
                st.caption("Suggested questions:")
                suggestions = [
                    "Which counties need most urgent help?",
                    "How is Kenya tracking to 2030 target?",
                    "How does the national aggregation work?",
                    "What does Makueni CEP show about clean cooking?",
                ]
                for i, s in enumerate(suggestions):
                    if st.button(s, key=f"ai_sidebar_sug_{i}",
                                 use_container_width=True):
            ...
            # Chat history — show last 6 messages in compact format
            if st.session_state["ai_history"]:
                for msg in st.session_state["ai_history"][-6:]:
                    if msg["role"] == "user":
                        st.markdown(
                            f"<div style='background:#1a6fa3;color:#fff;"
                            f"border-radius:10px 10px 2px 10px;padding:7px 10px;"
                            f"margin-bottom:6px;font-size:11px;line-height:1.5;"
                            f"text-align:right'>{msg['content']}</div>",
                            unsafe_allow_html=True
                        )
                    else:
                        st.markdown(
                            f"<div style='background:rgba(62,207,170,0.12);"
                            f"color:#e8e6e0;border-radius:2px 10px 10px 10px;"
                            f"padding:7px 10px;margin-bottom:6px;font-size:11px;"
                            f"line-height:1.5;border-left:2px solid #3ecfaa'>"
            ...
            user_q = st.text_input(
                "Your question:",
                placeholder="Ask about Kenya energy planning…",
                key="ai_sidebar_input",
                label_visibility="collapsed"
            )
            col_send, col_clear = st.columns([2, 1])
            if col_send.button("Send ➤", key="ai_sidebar_send",
                ...
                if user_q.strip():
                    ...
                    st.session_state["ai_history"].append(
                        {"role": "assistant", "content": reply})
                    st.rerun()
            if col_clear.button("Clear", key="ai_sidebar_clear",
                                use_container_width=True):
                st.session_state["ai_history"] = []
                st.rerun()
