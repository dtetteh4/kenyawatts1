...
def main():
    ...
    # Strip emoji and whitespace robustly — handles single/double spaces after emoji
    import re as _re
    key = _re.sub(r'^[𐀀-􏿿☀-➿⭐-⭕⌚-⌛⏩-⏳⏸-⏺▪-▫▶◀◻-◾☔-☕♈-♓♿⚓⚡⚪-⚫⚽-⚾⛄-⛅⛎⛔⛪⛲-⛳⛵⛺⛽✂✅✈-✍✏✒✔✖✝✡✨✳-✴❄❇❌❎❓-❕❗❣-❤➕-➗➡➰➿⤴-⤵⬅-⬇⬛-⬜⭐⭕〰〽㊗㊙🀄🃏🅰-🅱🅾-🅿🆎🆑-🆚🈁-🈂🈚🈯🈲-🈺🉐-🉑🌀-🌡🌤-🎓🎖-🎗🎙-🎛🎞-🏰🏳-🏵🏷-📽📿-🔽🕉-🕎🕐-🕧🕯-🕰🕳-🕺🖇🖊-🖍🖐🖕-🖖🖤-🖥🖨🖱-🖲🖼🗂-🗄🗑-🗓🗜-🗞🗡🗣🗨🗯🗳🗺-🙄🙅-🙏🚀-🛅🛋-🛒🛠-🛥🛩🛫-🛬🛰🛳-🛸🤐-🤞🤠-🤧🤰🤳-🤾🥀-🥋🥐-🥞🦀-🦑🧀🧐-🧦️︎⃣︀-️]+', '', selected).strip()
    # Also handle the double-space separator we use in nav labels
    if "  " in key:
        key = key.split("  ", 1)[-1].strip()
    key_lower = key.lower()

    # Route — check both the cleaned key and the raw selected string
    def _matches(*terms):
        return any(t.lower() in key_lower or t.lower() in selected.lower() for t in terms)

    # Render into a single container to prevent double-rendering
    with st.container():
        if _matches("National overview") and not _matches("Communications","Validation","Submit","Download","AI","Account","Makueni","INEP report","County map","47 counties","dashboard","County demand"):
            page_national_overview(role)
        elif _matches("County map") and not _matches("Communications","47 counties","demand data"):
