...
def save_submission(record):
    """Save a submission to Supabase cloud AND local JSON backup."""
    if USE_CLOUD_DB:
...
def compute_national():
    """
    Compute population-weighted national energy indicators from all
    counties that have status 'submitted' or 'review'.
    Returns a dictionary of national figures used in the overview dashboard.
    """
...
def page_national_overview(role):
    ...
    if role == "developer":
        alert("warn",
              "<b>Developer / demo session.</b> You have full access to all platform features. "
              "This account is for the NGDA team to test and demonstrate the platform. "
              "Do not use this login to submit real county data.")
...
def page_inbox(role, county_id, user_name):
    ...
    if role == "county":
        ...
        with tab_in:
            # ── Auto-refresh every 15 seconds so new EPRA messages appear ─────
            import time as _time
            all_msgs_now = get_messages()
            epra_msgs_for_county = [
                m for m in all_msgs_now
                if m.get("from_role") in ("epra","system")
                and (m.get("to","").lower() == cnty.lower()
                     or "all" in m.get("to","").lower())
            ]
            county_sent = [m for m in all_msgs_now if m.get("from_county")==cnty]
            replies_received = sum(len(m.get("replies",[])) for m in county_sent)

            # Unread count badge
            unread_epra = len(epra_msgs_for_county)
            if unread_epra > 0 or replies_received > 0:
                st.markdown(f"""
                <div style="display:flex;gap:10px;margin-bottom:12px;flex-wrap:wrap">
                  <div style="background:#0e1e2e;border-radius:8px;padding:8px 14px;font-size:12px;
                              border-left:3px solid #1a6fa3">
                    <span style="color:#a8c4d4">Messages from EPRA:</span>
                    <span style="color:#3ecfaa;font-weight:700;margin-left:6px">{unread_epra}</span>
                  </div>
                  {f'<div style="background:#0e1e2e;border-radius:8px;padding:8px 14px;font-size:12px;border-left:3px solid #0f9d7e"><span style="color:#a8c4d4">Replies from EPRA:</span><span style="color:#3ecfaa;font-weight:700;margin-left:6px">{replies_received}</span></div>' if replies_received > 0 else ""}
                </div>""", unsafe_allow_html=True)

            # Refresh button so county can pull latest messages manually
            rc1, rc2 = st.columns([1,4])
            if rc1.button("🔄 Refresh inbox", key="refresh_inbox_btn"):
                st.rerun()
            rc2.caption(f"Last checked: {datetime.now().strftime('%H:%M:%S')} · Messages update when you refresh")
    ...
    # ── EPRA COMMUNICATIONS HUB ───────────────────────────────────────────────
        ...
        with tab_recv:
            # ── LIVE COUNTY MESSAGES — refresh pulls from file ────────────────
            er1, er2 = st.columns([1,4])
            if er1.button("🔄 Refresh", key="epra_refresh_btn"):
                st.rerun()
            er2.caption(f"Last checked: {datetime.now().strftime('%H:%M:%S')} · New county messages appear when you refresh")

            county_msgs = [m for m in get_messages() if m.get("from_role") in ("county","system")]
        ...
        with tab_broadcast:
            # ── EPRA Broadcast Composer ───────────────────────────────────────
            # This tab lets EPRA compose and send messages to counties.
            # We avoid st.form() because it resets all fields on rerun.
            # Instead each widget has a unique key and we manage state manually.
            # The multiselect uses NO default= argument to avoid the silent
            # Streamlit error that causes a blank page when default values
            # do not exactly match the current options list.
            ...
            alert("info", (
                ...
                "and body, then click Send. The message appears immediately "
                "in the county Communications tab when they click Refresh."
            ))

            # ── Sample messages expander (for testing) ────────────────────────
            # Expand this to copy-paste ready test messages
            with st.expander("📋 Click here for sample test messages", expanded=True):
                st.markdown("""
**Copy any of these to test the two-way messaging:**

| Recipient | Subject | Body |
|---|---|---|
| Makueni | Updated solar GHI baseline | Please use 2,008 kWh/m² as the reference solar GHI value for Makueni County when completing your submission. |
| All 47 counties | Submission deadline — 30 June 2026 | All county energy plans must be submitted by 30 June 2026 under INEP Regulations 2025. Plans not submitted by this date will be flagged for escalation to the Council of Governors. |
| Turkana | Direct submission support available | EPRA can provide direct technical assistance for your plan submission. Contact Allan.Wairimu@epra.go.ke or call +254720850696 to arrange a support session. |
| Overdue counties only | Final reminder — immediate action required | Your County Energy Plan is now critically overdue. This is your final notice before escalation. Please submit immediately through KenyaWatts or contact EPRA. |
                """)
            ...
            # ── Step 1: Recipients ────────────────────────────────────────────
            # Build the full options list of regions and individual counties
            # NOTE: No default= argument — avoids blank page bug in Streamlit
            # where default values that don't match options cause silent failure
            recipient_options = [
                "All 47 counties",
                "Overdue counties only (Turkana, Marsabit, Mandera, Wajir)",
                "North East region (Garissa, Mandera, Marsabit, Wajir)",
                "Coast region (Mombasa, Kwale, Kilifi)",
                "Rift Valley region (Nakuru, Turkana, Kajiado)",
                "Nairobi region",
                "Nyanza region (Kisumu)",
            ] + sorted(COUNTIES["name"].tolist())  # Add all individual county names

            b_to = st.multiselect(
                "1. Send to (select one or more): *",
                options=recipient_options,
                # No default= here — prevents the blank page bug
                key="bc_to_v2",
                help="Select regions to send to all counties in that region, or pick individual counties by name."
            )

            # ── Step 2: Message type ──────────────────────────────────────────
            # The type appears as a coloured badge on the county's received message
            b_type = st.selectbox(
                "2. Message type: *",
                ["Assumptions", "Guidance", "Benchmark", "Alert", "Reminder"],
                key="bc_type_v2",
                help=(
                    "Assumptions = mandatory planning baselines all counties must use  |  "
                    "Guidance = instructions on how to complete the plan  |  "
                    "Benchmark = comparison data showing county vs national average  |  "
                    "Alert = urgent action required  |  "
                    "Reminder = deadline or compliance notice"
                )
            )
            ...
            b_subject = st.text_input(
                "3. Subject line: *",
                placeholder="e.g. Updated national planning assumptions — June 2026",
                key="bc_subject_v2"
            )
            ...
            b_body = st.text_area(
                ...
                placeholder=(
                    ...
                    "Include:\n"
                    "- What the county needs to know\n"
                    "- What action is required (if any)\n"
                    "- Any deadline or reference number\n"
            ...
            # Show live character and word count as feedback
            if b_body:
                st.caption(
                    f"{len(b_body)} characters · "
                    f"{len(b_body.split())} words"
                )
            ...
            b_attach = st.file_uploader(
                ...
                type=["pdf", "xlsx", "docx"],
                key="bc_attach_v2",
                help="Attach a PDF, Excel, or Word document. The filename is recorded on the message."
            )
            ...
            if st.button(
                "📣  Send broadcast now",
                type="primary",
                key="send_broadcast_v2",
                use_container_width=False
            ):
                # Validate — both subject and body are required
                if not b_subject or not b_body:
                    st.error("Please fill in both the Subject (step 3) and Message body (step 4).")
                elif not b_to:
                    st.error("Please select at least one recipient in step 1.")
                else:
                    # Work out who to send to based on selections
                    target_counties = []
                    for t in b_to:
                        if "All 47" in t:
                            ...
                            target_counties.append(t)
                    to_str = ", ".join(sorted(set(target_counties))) if target_counties else "All counties"

                    # Build the message record to save
                    broadcast_msg = {
                    ...
                    # Save to persistent file/cloud store
                    add_message(broadcast_msg)
                    # Show success confirmation
                    push_notification(f"Broadcast sent to: {to_str}", "📣")
                    st.success(f"✅ Broadcast sent to: **{to_str}**")
                    st.info(
                        f"Message type: **{b_type}** · "
                        f"Subject: **{b_subject}** · "
                        f"Recipients see this when they open Communications and click Refresh."
                    )
                    # Clear v2 form fields so the form is ready for next message
                    for k in ["bc_to_v2","bc_type_v2","bc_subject_v2",
                              "bc_body_v2","bc_attach_v2"]:
...
def main():
      ...
      <div style="display:flex;justify-content:space-between;align-items:center">
        <div class="kw-logo">Kenya<span>Watts</span>
          <span style="font-size:13px;font-weight:400;color:#5a8a9e;margin-left:14px">
            Digital Integrated National Energy Planning Platform · EPRA Kenya
          </span>
        </div>
    ...
    rl = {"epra":"EPRA Planner","ministry":"Ministry of Energy",
          "devpartner":"Development Partner","county":"County Committee",
          "service_provider":"KPLC",
          "developer":"Platform Developer"}.get(role,"User")
    ...
    if role in ("epra", "developer"):
        ...
        base_pages = [
            st.Page(_national_overview, title="National overview",                icon="📊", default=True),
            st.Page(_county_map,        title="County map",                       icon="🗺️"),
            st.Page(_all_counties,      title="All 47 counties",                  icon="🏛️"),
            st.Page(_validation_queue,  title="Validation queue",                 icon="✅"),
            st.Page(_inbox,             title="Communications hub",               icon="📥"),
            st.Page(_makueni_ref,       title="Makueni CEP — template reference",  icon="📄"),
            st.Page(_inep_report,       title="National INEP report",             icon="📋"),
            st.Page(_data_download,     title="Data download",                    icon="⬇️"),
            st.Page(_ai,                title="AI assistant",                     icon="🤖"),
            st.Page(_account,           title="Account settings",                 icon="⚙️"),
        ]
        # Add developer tools ONLY for the developer role — not for EPRA
        if role == "developer":
            pages = base_pages + [
                st.Page(_dev_tools, title="🛠️  Developer tools", icon="🔧"),
            ]
        else:
            pages = base_pages
    elif role == "ministry":
        ...
        pages = [
            st.Page(_county_dashboard,  title=f"{cname} dashboard", icon="🏠", default=True),
            st.Page(_submit,            title="Submit energy plan",  icon="📤"),
            st.Page(_inbox,             title="Communications",      icon="📨"),
            st.Page(_data_download,     title="Data download",       icon="⬇️"),
            st.Page(_ai,                title="AI assistant",        icon="🤖"),
            st.Page(_account,           title="Account settings",    icon="⚙️"),
        ]
        if is_makueni:
            pages.insert(2, st.Page(_makueni_ref, title="My county plan", icon="📄"))
    else:  # service_provider
        pages = [
            st.Page(_national_overview, title="National overview",      icon="📊", default=True),
            st.Page(_all_counties,      title="County demand data",     icon="🏛️"),
            st.Page(_submit,            title="Submit provider plan",   icon="📤"),
            st.Page(_data_download,     title="Data download",          icon="⬇️"),
            st.Page(_ai,                title="AI assistant",           icon="🤖"),
            st.Page(_account,           title="Account settings",       icon="⚙️"),
        ]
    ...
    with st.sidebar:
        ...
        if st.button("🚪  Sign out", use_container_width=True, key="signout_btn"):
            ...
            st.rerun()
        st.markdown("""<div style="font-size:10px;color:#9c9a8e;line-height:1.7;margin-top:8px">
        Data: EPRA FY 2024/25 · Makueni CEP 2023–2032 · KNBS 2019<br>
